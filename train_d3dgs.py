...
def get_loss(params, curr_data, variables, is_initial_timestep):
    ...
    #                 'soft_col_cons': 0.01}
    loss_weights = {'im': 1.0, 'seg': 0.0, 'rigid': 4.0, 'rot': 4.0, 'iso': 2.0, 'floor': 2.0, 'bg': 20.0,
                    'soft_col_cons': 0.01}
    loss = sum([loss_weights[k] * v for k, v in losses.items()])
    seen = radius > 0
    #出现报错，尝试debug
    # print("radius.shape:", radius.shape)
    # print("variables['max_2D_radius'].shape:", variables['max_2D_radius'].shape)
    # print("seen.shape:", seen.shape)
    # print("seen.sum():", seen.sum())  # 打印 True 值的数量
...
def train(seq, exp, args):
    if os.path.exists(f"./{args.output_dir}/{exp}/{seq}"):
    ...
    for t in range(num_timesteps):
        ...
        num_iter_per_timestep = args.num_initial_iters if is_initial_timestep else args.num_iters
...
if __name__ == "__main__":
    ...
    parser.add_argument("--num_initial_iters", type=int, default=10000)
