...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background,\
    no_fine, adaptive_motion_hierarchy, render_test=False, reconstruct=False, crop_size=0):
    ...
    gt_depths = []
    mask_list = []
    deformation_mask = np.load("deformation_mask.npy")
    deformation_mask = torch.tensor(deformation_mask)
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        stage = 'coarse' if no_fine else 'fine'
        rendering = render(view, gaussians, pipeline, background, adaptive_motion_hierarchy, mask = deformation_mask)
    ...
    if render_test:
        ...
        for i in range(test_times):
            ...
            if i == 60:
                time2 = time()
        print("FPS:", (len(views)-1) * (60 - 50 + 1)/ (time2 - time1))
...
def render_sets(dataset : ModelParams, hyperparam, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_test : bool, skip_video: bool, reconstruct_train: bool, reconstruct_test: bool, reconstruct_video: bool):
    with torch.no_grad():
        ...
        gaussians = GaussianModel(dataset.sh_degree, hyperparam, adaptive_motion_hierarchy)
        ...
        if not skip_train:
            render_set(dataset.model_path, "train", scene.loaded_iter, scene.getTrainCameras(), gaussians, pipeline, background, False, adaptive_motion_hierarchy, reconstruct=reconstruct_train)
        if not skip_test:
            render_set(dataset.model_path, "test", scene.loaded_iter, scene.getTestCameras(), gaussians, pipeline, background, False, adaptive_motion_hierarchy, reconstruct=reconstruct_test, crop_size=20)
        if not skip_video:
            render_set(dataset.model_path,"video",scene.loaded_iter, scene.getVideoCameras(),gaussians,pipeline,background, False, adaptive_motion_hierarchy, render_test=True, reconstruct=reconstruct_video, crop_size=20)
